...
export const crearApuestaSchema =   z.object({
    ...
    return Math.abs(suma - 100) <= 0.01; // Permite una pequeña tolerancia  
},{
    message: 'Las probabilidades deben sumas 100%',     
}).refine(data => {
    return data.probabilidades.every(p => p > 0 && p < 100);
},{
    message: 'Las probabilidades deben ser mayores a 0 y menores o iguales a 100',
});
